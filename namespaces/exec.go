...
func Exec(container *libcontainer.Config, stdin io.Reader, stdout, stderr io.Writer, console string, rootfs, dataPath string, args []string, createCommand CreateCommand, setupCommand SetupCommand, startCallback func()) (int, error) {
	var (
		err error
	)
	...
	// Start the helper process to setup the init process
	setupCmd := setupCommand(container, console, rootfs, dataPath, os.Args[0], args)
	...
	log.Println("After waiting for setup", setupCmd.ProcessState.Sys().(syscall.WaitStatus).ExitStatus())
	...
	// Sync with child (init)
	if err := syncPipe.ReadFromChild(); err != nil {
		command.Process.Kill()
		command.Wait()
		log.Println("Child read failed")
		return -1, err
	}

	log.Println("Child read done. Starting callback.")
	...
	if err := command.Wait(); err != nil {
		if _, ok := err.(*exec.ExitError); !ok {
			log.Println("Waiting failed")
			return -1, err
		}
	}

	log.Println("Exiting", command.ProcessState.Sys().(syscall.WaitStatus).ExitStatus())
...
// root: the path to the container json file and information
// args: the arguments to pass to the container to run as the user's program
func DefaultSetupCommand(container *libcontainer.Config, console, rootfs, dataPath, init string, args []string) *exec.Cmd {
	// get our binary name from arg0 so we can always reexec ourself
	env := []string{
		"setup=1",
		"console=" + console,
	...
	command := exec.Command(init, append([]string{"exec"}, args...)...)
	// make sure the process is executed inside the context of the rootfs
	command.Dir = rootfs
	command.Env = append(os.Environ(), env...)
